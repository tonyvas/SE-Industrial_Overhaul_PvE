...
namespace ReactorWaste_NR{													// Add/replace your reactor subtype IDs here
	
	[MyEntityComponentDescriptor(typeof(MyObjectBuilder_OxygenGenerator), false, "NuclearReactor")]
	 
	public class ReactorWaste_NR : MyGameLogicComponent{
		...
		public override void UpdateBeforeSimulation10(){
			...
			if(FirstRun == true){
				...
				if(CurrentFuelLevel == 0){
					...
					OxygenGenerator.Enabled = false;
					
				}
			
				FuelUsed = LastFuelLevel - CurrentFuelLevel; // Compares current fuel level against last known fuel level
				
				if(FuelUsed > 0 && FuelUsed <= 1){ // Limits values to prevent bad behaviour when adding or removing fuel normally
