...
namespace CementKilnSpinAnimation4
{
    // Edit the block type and subtypes to match your block.
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Assembler), false, "CementKiln")]
    public class CementKilnSpin4 : MyGameLogicComponent
    {
        ...
        public override void UpdateOnceBeforeFrame()
        {
            ...
            MatrixX = Matrix.CreateRotationX(0.18f); // rotation speed for each axis
            MatrixY = Matrix.CreateRotationY(0f);
            MatrixZ = Matrix.CreateRotationZ(0f);
			
			var Axis = new Vector3(0.9961947f, 0.0871557f, 0f); // (x, y, z) rotational axis
			
            MatrixAxis = Matrix.CreateFromAxisAngle(Axis, 0.01f);
        ...
        public override void UpdateBeforeSimulation()
        {
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    ...
                    if(shouldSpin)
					{		
						var offset = new Vector3(0.58f, 0.1f, 0f); // (x, y, z) offset from center of block
						...
						{
							MatrixTotal *= (MatrixTrans1 * MatrixAxis * MatrixTrans2);
							subpart.PositionComp.LocalMatrix = MatrixTotal;
						}
					}
				}
