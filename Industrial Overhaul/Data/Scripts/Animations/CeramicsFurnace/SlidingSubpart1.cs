...
namespace SiliconFuserSlideAnimation1
{
    // Edit the block type and subtypes to match your block.
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Assembler), false, "SiliconFuser")]
    public class SiliconFuserSlide1 : MyGameLogicComponent
    {
        ...
        private int SlideReverseTime = 400; // How long the subpart slides while reversing, in ticks
											 // In most cases should be the same as SlideTime
        private int CurrentSlideTime = 0;

        private float MovePerTick = -0.011f; // How fast the subpart slides, in meters per tick
        private float MoveReversePerTick = -0.011f; // How fast the subpart slides, in meters per tick
            ...
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    if(subpartFirstFind)
                    {
						...
						MatrixSlide = Matrix.CreateTranslation(2.2f, -0.58f, -3.775f); // (x, y, z) offset from center of block
                    }
