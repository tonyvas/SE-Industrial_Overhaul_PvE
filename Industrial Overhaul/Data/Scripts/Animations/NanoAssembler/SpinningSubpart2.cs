...
namespace NanoAssemblerSpinAnimation2
{
    // Edit the block type and subtypes to match your block.
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Assembler), false, "NanoAssembler")]
    public class NanoAssemblerSpin2 : MyGameLogicComponent
    {
        ...
        private int SpinTime = 400; // How long the subpart spins before reversing, in ticks
		
        private int SpinReverseTime = 400; // How long the subpart spins in reverse, in ticks
        ...
        public override void UpdateOnceBeforeFrame()
        {
            ...
            MatrixY = Matrix.CreateRotationY(0.003375f);
            MatrixZ = Matrix.CreateRotationZ(0f);

            MatrixXReverse = Matrix.CreateRotationX(0f); // reverse rotation speed for each axis
            MatrixYReverse = Matrix.CreateRotationY(-0.003375f); // in most cases should be the exact negative of the normal rotation
