...
namespace PlateStampSlideAnimation1
{
    // Edit the block type and subtypes to match your block.
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Assembler), false, "PlateStamp")]
    public class PlateStampSlide1 : MyGameLogicComponent
    {
        ...
        private int SlideTime = 40; // How long the subpart slides before reversing, in ticks
        private int SlideReverseTime = 10; // How long the subpart slides while reversing, in ticks
											 // In most cases should be the same as SlideTime
        private int CurrentSlideTime = 0;

        private float MovePerTick = 0.02f; // How fast the subpart slides, in meters per tick
        private float MoveReversePerTick = 0.08f; // How fast the subpart slides, in meters per tick
            ...
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    if(subpartFirstFind)
                    {
						...
						MatrixSlide = Matrix.CreateTranslation(-0.85f, -1.23265f, 0f); // (x, y, z) offset from center of block
                    }
