...
namespace AdvancedAssemblerSpinAnimation4
{
    // Edit the block type and subtypes to match your block.
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Assembler), false, "AdvancedAssembler")]
    public class AdvancedAssemblerSpin4 : MyGameLogicComponent
    {
        private const string SUBPART_NAME = "Spin4"; // dummy name without the "subpart_" prefix
        ...
        private int SpinTime = 230; // How long the subpart spins before reversing, in ticks
		
        private int SpinReverseTime = 230; // How long the subpart spins in reverse, in ticks
        ...
        public override void UpdateOnceBeforeFrame()
        {
            ...
            MatrixX = Matrix.CreateRotationX(0.00128998f); // rotation speed for each axis
            MatrixY = Matrix.CreateRotationY(0f);
            MatrixZ = Matrix.CreateRotationZ(-0.002f);

            MatrixXReverse = Matrix.CreateRotationX(-0.00128998f); // reverse rotation speed for each axis
            MatrixYReverse = Matrix.CreateRotationY(0f); // in most cases should be the exact negative of the normal rotation
            MatrixZReverse = Matrix.CreateRotationZ(0.002f);
        ...
        public override void UpdateBeforeSimulation()
        {
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    ...
                    if(shouldSpin)
					{		
						var offset = new Vector3(-2.5f, -1.85595f, -2f); // (x, y, z) offset from center of block
						var MatrixTrans1 = Matrix.CreateTranslation(-(offset)); // NOTE: SE axes may be different than 
