...
namespace MicroelectronicsFactorySlideAnimation1
{
    ...
    public class MicroelectronicsFactorySlide1 : MyGameLogicComponent
    {
        ...
        private int SlideTime = 200; // How long the subpart slides before reversing, in ticks
        private int SlideReverseTime = 200; // How long the subpart slides while reversing, in ticks
        ...
        private float MoveReversePerTick = 0.006f; // How fast the subpart slides, in meters per tick
            ...
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    if(subpartFirstFind)
                    {
						...
						MatrixSlide = Matrix.CreateTranslation(-0.75063f, 0.701371f, -0.6f); // (x, y, z) offset from center of block
                    }
