...
namespace ReprocessorSpinAnimation2
{
    ...
    public class ReprocessorSpin2 : MyGameLogicComponent
    {
        ...
        private int SpinTime = 260; // How long the subpart spins before reversing, in ticks
		
        private int SpinReverseTime = 260; // How long the subpart spins in reverse, in ticks
        ...
        public override void UpdateOnceBeforeFrame()
        {
            ...
            MatrixY = Matrix.CreateRotationY(-0.0025f);
            MatrixZ = Matrix.CreateRotationZ(0f);

            MatrixXReverse = Matrix.CreateRotationX(0f); // reverse rotation speed for each axis
            MatrixYReverse = Matrix.CreateRotationY(0.0025f); // in most cases should be the exact negative of the normal rotation
        ...
        public override void UpdateBeforeSimulation()
        {
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    ...
                    if(shouldSpin)
					{		
						var offset = new Vector3(1.50095f, 0.446645f, -0.518086f); // (x, y, z) offset from center of block
