...
namespace AutoLoomSlideAnimation2
{
    // Edit the block type and subtypes to match your block.
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Assembler), false, "AutoLoom")]
    public class AutoLoomSlide2 : MyGameLogicComponent
    {
        ...
        private int SlideTime = 18; // How long the subpart slides before reversing, in ticks
        private int SlideReverseTime = 18; // How long the subpart slides while reversing, in ticks
											 // In most cases should be the same as SlideTime
        private int CurrentSlideTime = 0;

        private float MovePerTick = 0.025f; // How fast the subpart slides, in meters per tick
        private float MoveReversePerTick = 0.025f; // How fast the subpart slides, in meters per tick
            ...
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    if(subpartFirstFind)
                    {
						...
						MatrixSlide = Matrix.CreateTranslation(0.65f, -0.325f, 0f); // (x, y, z) offset from center of block
                    }
