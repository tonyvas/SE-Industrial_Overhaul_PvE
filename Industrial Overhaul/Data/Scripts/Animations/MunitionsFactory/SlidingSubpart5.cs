...
namespace MunitionsFactorySlideAnimation5
{
    ...
    public class MunitionsFactorySlide5 : MyGameLogicComponent
    {
        private const string SUBPART_NAME = "Slide5"; // dummy name without the "subpart_" prefix
            ...
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    if(subpartFirstFind)
                    {
						...
						MatrixSlide = Matrix.CreateTranslation(-0.8f, -0.371f, 0.35f); // (x, y, z) offset from center of block
                    }
