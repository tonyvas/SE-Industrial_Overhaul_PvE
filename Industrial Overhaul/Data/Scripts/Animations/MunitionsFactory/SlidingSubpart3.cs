...
namespace MunitionsFactorySlideAnimation3
{
    ...
    public class MunitionsFactorySlide3 : MyGameLogicComponent
    {
        private const string SUBPART_NAME = "Slide3"; // dummy name without the "subpart_" prefix
            ...
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    if(subpartFirstFind)
                    {
						...
						MatrixSlide = Matrix.CreateTranslation(1.55f, -0.635075f, -0.1f); // (x, y, z) offset from center of block
                    }
