...
namespace BasicAssemblerSpinAnimation5
{
    // Edit the block type and subtypes to match your block.
    [MyEntityComponentDescriptor(typeof(MyObjectBuilder_Assembler), false, "BasicAssembler")]
    public class BasicAssemblerSpin5 : MyGameLogicComponent
    {
        private const string SUBPART_NAME = "Spin5"; // dummy name without the "subpart_" prefix
        ...
        private int SpinTime = 55; // If using reversing rotation, how long the subpart spins before reversing, in ticks
		
        private int SpinReverseTime = 55; // How long the subpart spins in reverse, in ticks
        ...
        public override void UpdateOnceBeforeFrame()
        {
            ...
            MatrixZ = Matrix.CreateRotationZ(0.01f);
            ...
            MatrixZReverse = Matrix.CreateRotationZ(-0.01f);
        ...
        public override void UpdateBeforeSimulation()
        {
            try
            {
                ...
                if(Entity.TryGetSubpart(SUBPART_NAME, out subpart))
                {
                    ...
                    if(shouldSpin)
					{		
						var offset = new Vector3(0.946924f, 0.468945f, -0.05f); // (x, y, z) offset from center of block
